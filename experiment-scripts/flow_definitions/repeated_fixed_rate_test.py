...
def variation_function(api, cfg, NCS_API_LOCATION, rate_min: int, rate_max: int, rate_step: int, 
    ...
    """
    Execute repeated tests on a single flow with fixed transmission rate.
    
    This function tests network flow performance by repeatedly transmitting traffic at a fixed
    rate. The rate is taken from rate_min (rate_max and rate_step are ignored in this mode).
    The number of repetitions is determined by rate_step (reused as test_count).
    Each test runs for flow_duration seconds and results are evaluated for packet/byte loss.
    
    IMPORTANT: For this test, the flow is deleted and recreated before each test iteration
    to reset metrics to 0, ensuring clean measurements for each repetition.
    ...
    Args:
        ...
        rate_min (int): Fixed transmission rate in Mbps (used as the test rate)
        rate_max (int): Not used in this mode (kept for compatibility)
        rate_step (int): Number of test repetitions (reused as test_count)
    ...
    def variation_worker():
        ...
        if len(cfg.flows) != 1:
            logger.error(f"Repeated fixed rate test requires exactly 1 flow, found {len(cfg.flows)}")
            return
        ...
        # Extract parameters
        fixed_rate_mbps = rate_min  # Use rate_min as the fixed rate
        test_count = rate_step      # Use rate_step as the number of repetitions
        
        # Generate timestamp for results file
        test_start_timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        
        logger.info(f"Starting repeated fixed rate test")
        logger.info(f"Fixed rate: {fixed_rate_mbps} Mbps")
        logger.info(f"Number of tests: {test_count}")
        logger.info(f"Test duration: {flow_duration}s, NCS delay: {ncs_to_flow_delay}s")
        logger.info(f"Note: Flow will be recreated before each test to reset metrics to 0")
        logger.info(f"Results will be saved to: results/{test_start_timestamp}.txt")
        
        # Iterate through all test repetitions
        for test_num in range(1, test_count + 1):
            if stop_event.is_set():
                logger.info("Repeated fixed rate test stopped by user")
                break
            
            logger.info(f"\n{'='*60}")
            logger.info(f"Test {test_num}/{test_count} at {fixed_rate_mbps} Mbps")
            logger.info(f"{'='*60}")
            ...
            flow.rate.mbps = fixed_rate_mbps
            ...
            api.set_config(cfg)
            logger.info(f"Flow recreated successfully, metrics reset to 0")
            ...
            # Start traffic
            logger.info(f"Starting traffic at {fixed_rate_mbps} Mbps...")
            ...
            # Log results
            logger.info(f"\nResults for test {test_num}/{test_count}:")
        ...
        summary_lines.append("REPEATED FIXED RATE TEST SUMMARY")
        summary_lines.append("="*60)
        summary_lines.append(f"Test Start Time: {test_start_timestamp}")
        summary_lines.append(f"Fixed Rate: {fixed_rate_mbps} Mbps")
        summary_lines.append(f"Packet Size: {use_packet_size} bytes")
        summary_lines.append(f"Test Duration: {flow_duration}s per test")
        summary_lines.append(f"Number of Tests: {test_count}")
        summary_lines.append("="*60)
        summary_lines.append(f"{'Test #':<8} {'Bytes TX':<12} {'Bytes RX':<12} {'Frames TX':<12} {'Frames RX':<12} {'Status':<8}")
        summary_lines.append("-"*60)
        
        for result in test_results:
            summary_lines.append(f"{result['test_num']:<8} {result['bytes_tx']:<12} {result['bytes_rx']:<12} "
        ...
        summary_lines.append(f"Total tests: {len(test_results)}, OK: {ok_count}, NOTOK: {notok_count}")
        
        # Calculate success rate
        if len(test_results) > 0:
            success_rate = (ok_count / len(test_results)) * 100
            summary_lines.append(f"Success rate: {success_rate:.1f}%")
        
        summary_lines.append("="*60)
        summary_lines.append(f"Test completed at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        ...
        try:
            ...
            results_file = os.path.join(results_dir, f"{test_start_timestamp}.txt")
        ...
        logger.info("Repeated fixed rate test completed")
